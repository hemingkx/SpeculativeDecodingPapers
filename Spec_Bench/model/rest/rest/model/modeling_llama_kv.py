...
# 2. Modifying attention mask for integration with Rest
# adapted from https://github.com/FasterDecoding/Medusa/blob/main/medusa/model/modeling_llama_kv.py
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        # [MODIFIED] add draft mask
        if hasattr(self, "draft_mask") and self.draft_mask is not None:
            draft_mask = self.draft_mask
            draft_len = draft_mask.size(-1)
            combined_attention_mask[:, :, -draft_len:, -draft_len:][
                draft_mask == 0
